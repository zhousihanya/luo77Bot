 ...
 * 群聊 @机器人 消息事件
 */
class GroupAtMessageEvent implements EventInterface
{
     ...
     * 获取群聊 openid
     */
    public function getGroupOpenid(): string
    {
        return $this->payload['group_openid'] ?? '';
    }

    /**
     * 获取发送者 member_openid
     */
    public function getMemberOpenid(): string
    {
        return $this->payload['author']['member_openid'] ?? '';
    }

    /**
     * 获取消息内容（纯文本，已去除 @ 机器人的内容）
     */
    ...
    public function reply(MessageInterface $message, ?int $msgSeq = null): array
    {
        ...
        return $this->client->sendGroupMessage($this->getGroupOpenid(), $data);
    }
    ...
    /**
     * 上传图片到群聊并返回 file_info
     */
    public function uploadImage(string $url): string
    {
        return $this->getUploader()->uploadGroupImage($this->getGroupOpenid(), $url);
    }

    /**
     * 上传视频到群聊并返回 file_info
     */
    public function uploadVideo(string $url): string
    {
        return $this->getUploader()->uploadGroupVideo($this->getGroupOpenid(), $url);
    }

    /**
     * 上传语音到群聊并返回 file_info
     */
    public function uploadAudio(string $url): string
    {
        return $this->getUploader()->uploadGroupAudio($this->getGroupOpenid(), $url);
    }

    /**
     * 上传文件到群聊并返回 file_info
     *
    ...
    {
        return $this->getUploader()->uploadGroupFile($this->getGroupOpenid(), $url, $fileName);
    }
